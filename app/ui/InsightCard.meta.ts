...
export const meta: DsMeta = {
  ...
  useWhen: [
    ...
    "Compose, don't wrap: `InsightCard` shell → `InsightCardHeader` (sparkle + company name + `InsightStatusPill` + regenerate) → `oi-row` grids of `InsightSection`s holding `RiskCard`/`WinRow`/`FocusTodayItem`/`ThisWeekRow`/`EmptyMessage`.",
  ...
  a11y: [
    ...
    "HONEST GAPS (audit Lens 3): the regenerate button only swaps its glyph to '…' while streaming — no `aria-busy`, so the streaming state is not announced; WinRow/ThisWeekRow position styling is visual grouping only (no list semantics).",
  ],
