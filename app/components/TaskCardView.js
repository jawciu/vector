...
/**
 * Pure presentational task card. No drag-and-drop wiring, no API calls —
 * just visual rendering driven by props.
 *
 * The kanban's `TaskCard` wraps this with `useSortable` + the toggle-done
 * API call. Other surfaces (e.g. `InsightsPanel` Focus today) render this
 * directly with their own `onCardClick` handler and no toggle behaviour.
 *
 * Props:
 *   - task           Full task object (title, status, due, owner, priority,
 *                    blockedByTask, notes, files, commentCount).
 ...
 *                    `style` here. Other callers can leave this empty.
 */
...
export default function TaskCardView({
  ...
  // SSR-safe gate for "today"-relative date labels — avoids hydration drift
  // when the day boundary crosses between server render and client mount.
  const [mounted, setMounted] = useState(false);
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => setMounted(true), []);

  const isDone = task.status === "Done" || isCompleting;
  const statusColor = STATUS_COLORS[task.status] || "var(--text-secondary)";
  ...
  const { className: extraClassName, ...restWrapper } = wrapperProps;
  ...
  return (
    <div
      ...
      className={`task-card rounded-lg${isCompleting ? " task-completing" : ""}${
