...
import TaskTick from "@/app/ui/TaskTick";
...
export default function PortalTaskCard({ task, onTaskUpdated, onCardClick, onSessionExpired }) {
  ...
  async function handleToggleDone() {
    ...
    const body = task.status === "Done"
      ? { status: task.previousStatus || "Not started" }
    ...
    try {
      ...
      if (goingToDone) {
        setTimeout(() => {
          if (onTaskUpdated) onTaskUpdated(task.id, updatedTask);
          setCompleting(false);
        }, 800);
      } else {
        if (onTaskUpdated) onTaskUpdated(task.id, updatedTask);
  ...
  return (
    ...
    >
      ...
      {/* Row 4: Files + comments + priority + owner */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {task.commentCount > 0 && (
            <div className="flex items-center gap-1">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ color: "var(--text-secondary)" }}>
                <g clipPath="url(#pc-comment-clip)">
