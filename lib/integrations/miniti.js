...
export async function processMinitiEvent(eventId, onboardingId) {
  ...
  // Best-effort. If extraction fails we still try Pass 2 (it can fall back
  // on the transcript directly), but we log the failure so the operator
  // can see it on the debug surface.
  let extraction = null;
  try {
  ...
  // Persist Pass 1 ASAP so the debug viewer shows it even if Pass 2 fails.
