...
export async function processMinitiEvent(eventId, onboardingId) {
  ...
  // Persist orchestrator I/O for the debug viewer. Stored as JSON:
  //   input  = the context object we sent to Claude
  //   output = the array of tool calls Claude returned (tool/input/id)
  // Best-effort — a failure here must not block draft creation.
  await setExternalEventOrchestratorIO(eventId, {
    input: context,
    output: result.calls,
  }).catch((err) =>
    console.warn("[miniti] persist orchestrator IO failed", err)
  );
