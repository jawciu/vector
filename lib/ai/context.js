...
/**
 * Bumped when the shape/content of the AI context changes in a way that
 * should invalidate cached insights. Old caches naturally miss after a bump.
 *
 * History:
 *   1 — initial
 *   2 — Phase 4c: task entries now carry `taskId`; AI prompts reference
 *       tasks as "AC-12" instead of by title alone. Forces all cached
 *       insights to regenerate on first read after deploy.
 */
...
export async function buildOnboardingSnapshot(onboardingId, { today = new Date() } = {}) {
  const [onboarding, tasks, contacts, phases, magicLinks] = await Promise.all([
    ...
    getMagicLinksForOnboarding(onboardingId),
  ]);
  ...
  // taskId is the human-readable identifier (e.g. "AC-12") composed by
  // lib/db.js#mapTask — passing it through lets Claude reference tasks
  // as "AC-12" instead of by title alone.
  ...
  // Contacts with portal context — for customer-facing summaries that may
  // reference other portal users by name. `hasPortalAccess` reflects
  // whether the contact has a non-revoked, non-expired magic link.
  const activeContactIds = new Set(
    (magicLinks ?? [])
      .filter((l) => !l.revokedAt && new Date(l.expiresAt) > now)
      .map((l) => l.contactId)
  );
  const openTasksByContact = new Map();
  for (const t of tasks) {
    if (t.status === "Done" || !t.assigneeContactId) continue;
    openTasksByContact.set(t.assigneeContactId, (openTasksByContact.get(t.assigneeContactId) ?? 0) + 1);
  }
  const contactsEnriched = contacts.map((c) => ({
    id: c.id,
    name: c.name,
    role: c.role || null,
    hasPortalAccess: activeContactIds.has(c.id),
    openTaskCount: openTasksByContact.get(c.id) ?? 0,
  }));
