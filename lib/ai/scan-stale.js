 ...
 * @returns {Promise<{ scanned, drafted, skippedExisting, failed, byOnboarding, errors }>}
 *   `skippedExisting` = task already had a pending draft (no work needed).
 *   `failed` = Claude call or DB write threw — review console / errors.
 */
export async function scanStaleTasks({ scopeVendorId = null } = {}) {
    ...
    for (const t of stale) {
      ...
      try {
        ...
        if (draftId != null) {
          ...
          // generateDraftForTask returned null — getTaskForFollowup couldn't
          // find the task (deleted between fetch and process).
          failed += 1;
          errors.push({ taskId: t.id, reason: "task vanished mid-scan" });
        }
      } catch (err) {
        console.error(`[scan-stale] task ${t.id} failed:`, err);
        failed += 1;
        errors.push({ taskId: t.id, reason: err?.message ?? "unknown error" });
      }
    }
  ...
  return { scanned, drafted, skippedExisting, failed, byOnboarding, errors };
}
...
async function generateDraftForTask(taskId, staleness, onboardingId, today) {
  ...
  const draft = await createPendingAIChange({
    ...
    // Real FK column — this is what the visibility filters read. The copy in
    // `payload.ownerId` below is legacy and kept only until the readers that
    // still parse it are gone.
    ownerId,
    payload: {
      taskId,
      ownerId,
