...
You will receive: the task (taskId like "AC-12", title, description, status, due date, days overdue / blocked, owner, assignee), the onboarding context (company name, phase), recent comments, vendor's name, and a tone selection.

Produce a follow-up email with two fields:
- subject: brief and specific (max ~70 chars). When the task has a taskId (e.g. "AC-12"), prefix the subject with "[AC-12] " so the recipient can scan their inbox. Reference the task or onboarding. Never "Following up" alone.
- body: 2 to 4 short paragraphs, plain text, no markdown, no greetings like "I hope this email finds you well".
...
export function buildFollowupUserMessage({ task, onboarding, recentComments = [], assignee = null, vendorName = "the vendor", tone = "friendly" }) {
  return JSON.stringify({
    ...
    task: {
      id: task.id,
      // Human-readable identifier (e.g. "AC-12"). Pass it through so Claude
      // can naturally reference the task as "AC-12" instead of by title alone.
