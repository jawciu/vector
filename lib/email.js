 ...
 *  `taskCode` is the human-readable identifier (e.g. "AC-12"); when present
 *  it's prefixed to the subject so the recipient can scan their inbox. */
export async function sendTaskAssigned({ to, contactName, companyName, taskTitle, taskCode = null, token }) {
  if (!to) return { ok: false, error: "no_email" };
  const portalUrl = buildPortalUrl(token);
  const prefix = taskCode ? `[${taskCode}] ` : "";
  const subject = `${prefix}New task on your ${companyName} onboarding: "${taskTitle}"`;
  const text =
    `Hi ${contactName || "there"},\n\n` +
 ...
 *  `taskCode` is the human-readable identifier (e.g. "AC-12"); when present
 *  it's prefixed to the subject so the recipient can scan their inbox. */
export async function sendTaskCommented({ to, contactName, companyName, taskTitle, taskCode = null, authorName, excerpt, token }) {
  if (!to) return { ok: false, error: "no_email" };
  const portalUrl = buildPortalUrl(token);
  const prefix = taskCode ? `[${taskCode}] ` : "";
  const subject = `${prefix}${authorName} commented on "${taskTitle}"`;
