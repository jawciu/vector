...
<html><body style="margin:0;padding:24px;background:#f6f6f4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1a1a1a;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;padding:32px;">
    ...
    ${lines.map((l) => `<p style="margin:0 0 16px;font-size:15px;line-height:1.5;">${l}</p>`).join("")}
    <p style="margin:24px 0;"><a href="${escapeHtml(ctaUrl)}" style="display:inline-block;background:#1a1a1a;color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:8px;font-size:14px;font-weight:500;">${escapeHtml(ctaLabel)}</a></p>
    ${footer ? `<p style="margin:0;font-size:13px;color:#6b6b6b;">${escapeHtml(footer)}</p>` : ""}
  </div>
</body></html>`;
}
...
export async function sendTaskCommented({ to, contactName, companyName, taskTitle, authorName, excerpt, token }) {
  ...
  if (excerpt) {
    htmlBody.push(
      `<span style="display:block;padding:12px 16px;background:#f6f6f4;border-left:3px solid #d4d4d0;border-radius:4px;color:#4b4b4b;">${escapeHtml(excerpt)}</span>`
    );
  }
  const html = renderEmail({
    greeting: `Hi ${contactName || "there"},`,
