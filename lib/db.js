 ...
 * contact last visited. No per-row read state; the whole list clears when
 * lastSeenPortalAt advances. If the contact has never visited, falls back to
 * the last 7 days so the first banner isn't empty but also isn't a flood.
 */
...
export async function updateTask(id, data, { actor } = {}) {
  ...
  const existing = await prisma.task.findUnique({
    where: { id: taskId },
    select: { id: true, status: true, onboardingId: true, title: true },
  });
