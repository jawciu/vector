...
export async function listStuckEvents({ source = "miniti", limit = 20 } = {}) {
  ...
  return rows.map((r) => ({
    ...
    // Debug surface for the "Show debug" toggle. Both nullable.
...
/** Persist the orchestrator I/O for debugging. Both columns are nullable
 *  JSONB so pre-existing rows + ambiguous events can leave them null. */
export async function setExternalEventOrchestratorIO(id, { input, output }) {
  const eId = Number(id);
  if (Number.isNaN(eId)) return null;
  return prisma.externalEvent.update({
    where: { id: eId },
    data: {
      orchestratorInput: input ?? null,
      orchestratorOutput: output ?? null,
    },
  });
}
