...
export async function createComment(taskId, author, body) {
  const id = Number(taskId);
...
export async function createMagicLinksBulk(contactIds, onboardingId, expiresInDays = 30) {
  const oId = Number(onboardingId);
  ...
  const toCreate = ids.filter((id) => !activeSet.has(id));
  const skipped = ids
...
export async function getPortalTasks(onboardingId, contactId) {
  ...
  // Look up the contact's name so we can also match by the owner text field
  let contactName = null;
  if (!Number.isNaN(cId)) {
    const contact = await prisma.contact.findUnique({ where: { id: cId }, select: { name: true } });
  ...
  return tasks.map((t) => ({
    ...
    isAssignedToMe:
      (!Number.isNaN(cId) && t.assigneeContactId === cId) ||
      (contactName && t.owner && t.owner.toLowerCase() === contactName.toLowerCase()),
...
export async function updateTaskAsPortalUser(taskId, onboardingId, data) {
...
export async function createCommentAsPortalUser(taskId, onboardingId, contactName, body) {
  const tId = Number(taskId);
  const oId = Number(onboardingId);
  if (Number.isNaN(tId) || Number.isNaN(oId)) return null;
...
export async function createFile({ taskId, onboardingId, contactId, uploadedBy, fileName, fileSize, mimeType, storagePath }) {
  const tId = Number(taskId);
  const oId = Number(onboardingId);
  if (Number.isNaN(tId) || Number.isNaN(oId)) return null;
