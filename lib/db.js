...
export async function getIntegrationStats({ source = "miniti", days = 30 } = {}) {
  ...
  let inFlight = 0;    // matched, unprocessed, no error, but younger than the cutoff (probably still running)
  for (const r of rows) {
    if (r.error) {
      errored += 1;
      continue;
...
export async function listPendingAIChanges({ status = "pending", limit = 100 } = {}) {
...
/** Count of pending drafts (for the sidebar badge). */
export async function countPendingAIChanges() {
