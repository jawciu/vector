 ...
 * and getUser() — token refresh depends on it.
 *
 * Why getUser() and not getClaims(): this project signs JWTs with an asymmetric
 * key (ES256), so getClaims() verifies the token locally and never asks the auth
 * server whether the session behind it still exists. A session revoked
 * server-side (a sign-out elsewhere on the shared demo account, say) keeps an
 * unexpired access token for up to an hour, so getClaims() waves it through,
 * the proxy skips its demo re-login, and every route handler answers 401
 * because they all check with getUser(). Verifying here closes that gap: the
 * proxy now enforces exactly what the route handlers assume it enforced.
 */
