...
See `.claude/skills/design-system/SKILL.md` for full details.

### Component locations
- **`app/ui/`** — Design system primitives: `Button`, `IconButton`, `CalendarDropdown`, `Icons`, `FieldPill`, `FieldRow`
- **`app/components/`** — Feature components: `TaskCard`, `Sidebar`, `Menu`, `TaskDrawer`, etc.

### Key conventions
- **Check existing DS components before creating new UI elements.** Reference Figma designs when available.
- **All dropdowns/popovers MUST use `MenuList` + `MenuOption`** from `app/components/Menu.js`.
- **`IconButton` pattern**: always `w-5 h-5 rounded` (NOT `rounded-full`) + `.icon-btn` class. Add `.icon-btn--active` when open. `rounded-full` is ONLY for avatar/initials circles.
- **`Button` component**: `variant="primary"` uses `.btn-primary` CSS class with `--action` / `--action-hover` / `--action-active` / `--action-disabled` tokens.
- **Color tokens**: menu hover = `var(--bg-hover)`, active = `var(--surface-hover)`, container bg = `var(--bg)`.
